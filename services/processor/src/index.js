...
let noProcessedMessages = 0; //Låter kön vara tom.
const instanceId = crypto.randomUUID(); //Genererar ett random instance ID
...
const processor = async () => {
  while (running) {
    const out = await client.send(new ReceiveMessageCommand({
      ...
      WaitTimeSeconds: 15
    }));    //Väntar på meddelanden i kön
    
    if (!running) {
      console.log('Processor shutting down...');
      break;
    } //Ifall inte körs stängs ner
